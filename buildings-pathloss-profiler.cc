...
main (int argc, char *argv[])
{
  ...
  std::string name;
  name = std::to_string(enbIndoor) + "_building.out";
  ...
  if (enbIndoor)
    {
      //Ptr<Building> building1 = CreateObject<Building> (-2, 4, -2, 2, 0.0, 20.0);
      Ptr<Building> building1 = CreateObject<Building> ();
      building1->SetBoundaries (Box (-2, 2, -2, 2, 0, 20.0));
  ...
  propagationLossModel->SetAttribute ("ShadowSigmaOutdoor", DoubleValue (1.0));
  propagationLossModel->SetAttribute ("ShadowSigmaIndoor", DoubleValue (1.0));
  propagationLossModel->SetAttribute ("ShadowSigmaExtWalls", DoubleValue (1.0));
